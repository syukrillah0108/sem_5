<?php
    ...
    if (isset($_FILES['url_image']) && $_FILES['url_image']['error'] === UPLOAD_ERR_OK) {
        //File gambar dikirimkan, tanggal unggahan
        ...
        $statement = $database_connection->prepare("UPDATE `news_katalog` SET `title`=?, `desc`=?, `img`=?, `date`=? WHERE `id`=?");
        $statement->execute([$title, $content, $upload_directory . $namafile, $date, $id]);
    } else {
        //File gambar tidak dikirimkan, tangani tanpa file gambar
        $statement = $database_connection->prepare("UPDATE `news_katalog` SET `title`=?, `desc`=?, `date`=? WHERE `id`=?");
