<?php
    ...
    if (isset($username) && isset($password)) {
        ...
        $statement = $database_connection->prepare("SELECT id, username, password FROM data_user WHERE username = ?");
        ...
        if ($user && sha1($password) === $user['password']) {
            ...
            $updateStatement = $database_connection->prepare("UPDATE data_user SET session_token = ? WHERE id = ?");
